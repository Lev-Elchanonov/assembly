section .data
	...
	sort_direction: db SORT_DIRECTION
...
row_loop:
	...
	push	 rsi			;сохранили адрес начала текущей строки
	...
	call	heap_sort_row
	pop 	rsi			;возвращаем инфу о начале матрицы
	xor	rax, rax
	...
	syscall
heap_sort_row:
	push	rbp			;запушили rbp чтобы определить точку старта в стеке
	mov	rbp, rsp		;rbp переместили до верхушку стека
	
	push	rbx
	push	r12
	push	r13
	
	mov 	r12, rdi		;r12 - адрес начала текущей строки
	mov	r13, rdx		;r13 - длина строки
...
build_heap:
	cmp 	rcx, 0			
	jl	sort_start		;если rсx < 0, то мы просмотрели все родительские узлы, а значит можно перейти к сортировке
	
	mov	rdi, r12		;поместили в rdi инфу о текущей строке
...
heapify:
	...
	push	r14
	push 	r15
	push 	rcx			;запонимаем текущий корневой узел

	mov	r12, rdi		;r12 - адрес строки
	mov	r13, rsi		;r13 - адрес текущего элемента
	mov	r14, rdx		;r14 - длина строки
	
	mov 	rbx, r13		;rbx - индекс текущего корня
	
	;потом вычисляем его потомков
	mov	r15, r13
	shl	r15, 1			;r15 = 2*i
	inc	r15			;r15 = 2*i+1 индекс левого потомка
	...
	movsx	rax, byte [r12 + rbx]	;rax - значение текущего элемента
	movsx	rdx, byte [r12 + r15]	;rdx - значение левого потомка
	...
	jge	check_right		;если текущий >= левого, то их не меняем, переходим к проверке правого потомка
...
descending_left:	;ПО УБЫВАНИЮ
	...
	mov	rbx, r15		;иначе левый потомок меньше корня, надо обновить индекс

check_right:
	cmp	rcx, r14		;сравниваем правого потомка с родителем
	jge 	compare			;если правый >= родителя, 
	
	movsx	rax, byte [r12 + rbx]	;rax - значение текущего элемента (или левого потомка если мы его загрузили)
	movsx 	rdx, byte [r12 + rcx] 	;rdx - значение правого потомка

	cmp	byte [sort_direction], 0
	jne	descending_right	

	cmp	al, dl			;сравниваем младшие байты числа и его правого ребенка
	jge	compare			;если текущий >= правого, то все ок, переходим к сравнению
	mov	rbx, rcx		;обновляем индекс корня (корень должен быть больше любого из детей)
	jmp	compare			;теперь в корне содержится индекс максимального элемента
...
compare:	;в этой обалсти мы сравниваем пересчитанные индексы и меняем местами элементы если нужно
	cmp	rbx, r13 		;сравниваем индекс максимального  и индекс корня 
	...
	mov	rdi, r12		
	mov	rsi, rbx		;rsi - индекс куда переместили элемент
